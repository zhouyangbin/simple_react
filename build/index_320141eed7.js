...
"use strict";
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "c", function() { return recyclables; });
/* harmony export (immutable) */ __webpack_exports__["b"] = mergeState;
...
"use strict";
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "b", function() { return mayQueue; });
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "a", function() { return lifeCycleQueue; });
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0__MayDom__ = __webpack_require__(4);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1__Refs__ = __webpack_require__(2);

 //mayQueue 保存render过程中的各种事件队列 

var mayQueue = {
  dirtyComponentsQueue: [],
  //setState 需要diff的component队列 
  callbackQueue: [],
  //回调队列 setState 中的事件回调
  lifeCycleQueue: [],
  //生命周期过程中的回调队列 DidUpdate DidMount ref回调
  isInEvent: false,
  //是否在触发事件 回调事件中的setstate合并触发
  clearQueue: clearQueue,
  flushUpdates: flushUpdates //存放生命周期中的 DidMount DidUpdate以及ref回调
...
function clearQueue() {
  //ComponentDidMount
  clearLifeCycleQueue(); //如有有dirty Component diff

  flushUpdates(); //setState传入的回调函数
...
function flushUpdates() {
  var instance;
  var i = 0; //如果在当前生命周期的DidMount调用setState 放到下一生命周期处理
  ...
  while (instance = mayQueue.dirtyComponentsQueue.shift()) {
    if (i++ === 0) {
      __WEBPACK_IMPORTED_MODULE_1__Refs__["a" /* Refs */].isRoot = true;
    }

    if (instance.mayInst.dirty) {
      //如果C是脏组件diff 如果其在diff过程中子组件也需要diff diff之后
      //子组件_dirty会为false 没必要再diff一次；
      Object(__WEBPACK_IMPORTED_MODULE_0__MayDom__["b" /* reRender */])(instance);
    }
  ...
  clearLifeCycleQueue(); //防止setState currentOwner混乱

  __WEBPACK_IMPORTED_MODULE_1__Refs__["a" /* Refs */].currentOwner = null;
}

function clearLifeCycleQueue() {
  //先清空 生命周期 ref 的回调函数
  if (mayQueue.lifeCycleQueue && mayQueue.lifeCycleQueue.length > 0) {
    var callback; //其实ref很像生命周期函数,它比较特殊的地方在于vnode.type='div'之类的vnode
...
"use strict";
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "a", function() { return Refs; });
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0__may_dom_scheduler__ = __webpack_require__(1);
...
var Refs = {
  ...
  attachRef: function (vnode, hostNode) {
    if (vnode.refType === 1) {
      ...
      __WEBPACK_IMPORTED_MODULE_0__may_dom_scheduler__["a" /* lifeCycleQueue */].push(vnode.ref.bind(vnode, hostNode));
    } else if (vnode.refType === 2) {
      //string
...
"use strict";
/* harmony export (immutable) */ __webpack_exports__["a"] = getChildContext;
/* harmony export (immutable) */ __webpack_exports__["b"] = getContextByTypes;
/**
...
/* harmony export (immutable) */ __webpack_exports__["b"] = reRender;
/* harmony export (immutable) */ __webpack_exports__["d"] = unmountComponentAtNode;
/* harmony export (immutable) */ __webpack_exports__["a"] = findDOMNode;
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0__util__ = __webpack_require__(0);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1__scheduler__ = __webpack_require__(1);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_2__mountStrategy__ = __webpack_require__(11);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_3__diffStrategy__ = __webpack_require__(16);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_4__diffProps__ = __webpack_require__(5);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_5__Refs__ = __webpack_require__(2);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_6__DOMNamespaces__ = __webpack_require__(6);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_7__context__ = __webpack_require__(3);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_8__transformChildren__ = __webpack_require__(7);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_9__dispose__ = __webpack_require__(12);
...
var renderByMay = function (vnode, container, callback) {
  ...
  if (lastVnode) {
    //update
    __WEBPACK_IMPORTED_MODULE_5__Refs__["a" /* Refs */].isRoot = true;
    ...
    if (vnode && vnode.type) {
      ...
      __WEBPACK_IMPORTED_MODULE_5__Refs__["a" /* Refs */].isRoot = true;
      rootDom = __WEBPACK_IMPORTED_MODULE_2__mountStrategy__["a" /* mountStrategy */][vnode.mtype](vnode, isSVG);
  ...
  result = instance && !instance.mayInst.stateless && instance || rootDom; //执行render过程中的回调函数lifeCycle ref等

  __WEBPACK_IMPORTED_MODULE_1__scheduler__["b" /* mayQueue */].clearQueue();
  ...
  if (instance) {
    ...
    instance.mayInst.lifeState = 0;
    instance.mayInst.hostNode = rootDom; //当我开始着手解决 层层嵌套的component 最底层的那个setState触发的时候lifeState并不为0
  ...
  __WEBPACK_IMPORTED_MODULE_5__Refs__["a" /* Refs */].isRoot = false;
  __WEBPACK_IMPORTED_MODULE_5__Refs__["a" /* Refs */].currentOwner = null;
...
function reRender(instance) {
  ...
  var skip = false; //lifeState为3组件开始diff
  //WillReceive WillUpdate render DidUpdate等周期setState放到下一周期;

  instance.mayInst.lifeState = 3;
  var newState = Object(__WEBPACK_IMPORTED_MODULE_0__util__["b" /* mergeState */])(instance); //forceUpdate时 忽略shouldComponentUpdate
  ...
  instance.state = newState; //getChildContext有可能setState 所以需要newState再调用

  if (instance.getChildContext) {
    context = Object(__WEBPACK_IMPORTED_MODULE_7__context__["a" /* getChildContext */])(instance, context);
  }
  ...
  if (__WEBPACK_IMPORTED_MODULE_5__Refs__["a" /* Refs */].isRoot) {
    __WEBPACK_IMPORTED_MODULE_5__Refs__["a" /* Refs */].currentOwner = instance;
    __WEBPACK_IMPORTED_MODULE_5__Refs__["a" /* Refs */].isRoot = false;
  }
  ...
  if (!updated) {
    Object(__WEBPACK_IMPORTED_MODULE_9__dispose__["b" /* disposeVnode */])(prevRendered);
    return;
  }

  if (!isEmpty && Object(__WEBPACK_IMPORTED_MODULE_3__diffStrategy__["a" /* isSameType */])(prevRendered, updated)) {
    // updated.mayInfo.instance = instance;
    // updated.mayInfo.hostNode = hostNode;
    hostNode = __WEBPACK_IMPORTED_MODULE_3__diffStrategy__["b" /* updateStrategy */][updated.mtype](prevRendered, updated); //mtype === 1在这在设置instance会循环引用
    // updated.mtype === 2 && ();

    updated.mayInfo.vChildren = Object(__WEBPACK_IMPORTED_MODULE_8__transformChildren__["b" /* transformChildren */])(updated, hostNode);
    instance.mayInst.forceUpdate = null; //原先这种diffProps 再diffChildren的方式并未考虑到 render返回之后还是
    ...
    var dom = __WEBPACK_IMPORTED_MODULE_2__mountStrategy__["a" /* mountStrategy */][updated.mtype](updated, isSVG); //component上的hostNode保持最新
    // var lastVnode = hostNode.parentNode._lastVnode;
    // lastVnode && (lastVnode.mayInfo.hostNode = dom);

    hostNode.parentNode.replaceChild(dom, hostNode); // updated.mayInfo.hostNode = dom;
  ...
  if (instance.componentDidUpdate) {
    instance.componentDidUpdate(prevProps, prevState, instance.context); // lifeCycleQueue.push(instance.componentDidUpdate.bind(instance, prevProps, prevState, instance.context));
  } else {
    instance.mayInst.lifeState = 0;
...
function mayUpdate(prevVnode, newVnode, parent) {
  ...
  if (Object(__WEBPACK_IMPORTED_MODULE_3__diffStrategy__["a" /* isSameType */])(prevVnode, newVnode)) {
    dom = __WEBPACK_IMPORTED_MODULE_3__diffStrategy__["b" /* updateStrategy */][newVnode.mtype](prevVnode, newVnode);
  } else {
    var isSVG = newVnode.mtype === 3;
    dom = __WEBPACK_IMPORTED_MODULE_2__mountStrategy__["a" /* mountStrategy */][newVnode.mtype](newVnode, isSVG);
    ...
    Object(__WEBPACK_IMPORTED_MODULE_9__dispose__["b" /* disposeVnode */])(prevVnode);
    hostNode = null;
...
function unmountComponentAtNode(dom) {
  ...
  if (lastVnode) {
    Object(__WEBPACK_IMPORTED_MODULE_9__dispose__["b" /* disposeVnode */])(lastVnode);
    Object(__WEBPACK_IMPORTED_MODULE_9__dispose__["c" /* emptyElement */])(dom); //unmount之后又render
...
"use strict";
/* harmony export (immutable) */ __webpack_exports__["b"] = diffProps;
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "a", function() { return FormElement; });
/* unused harmony export setDomAttr */
/* unused harmony export removeDomAttr */
/* unused harmony export patchStyle */
/* harmony export (immutable) */ __webpack_exports__["c"] = getIsControlled;
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0__event__ = __webpack_require__(14);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1__Refs__ = __webpack_require__(2);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_2__may_dom_scheduler__ = __webpack_require__(1);
...
function setDomAttr(dom, key, val) {
  var nodeType = dom.nodeType; // Don't get/set attributes on text, comment and attribute nodes
  ...
  if (!isEvent(key)) {
    ...
    var eventName = Object(__WEBPACK_IMPORTED_MODULE_0__event__["c" /* getBrowserName */])(key);
    var listener = dom._listener || (dom._listener = {});

    if (!listener[e]) {
      //添加过一次之后不必再添加;
      Object(__WEBPACK_IMPORTED_MODULE_0__event__["a" /* addEvent */])(eventName);
      var hook = __WEBPACK_IMPORTED_MODULE_0__event__["b" /* eventHooks */][eventName];
...
function patchStyle(dom, prevStyle, newStyle) {
  ...
  for (var name in newStyle) {
    var _type = typeof newStyle[name]; //backgroundColor 替换为 background-color Webkit替换为-webkit-   ms单独替换一次
...
function getIsControlled(hostNode, vnode) {
  ...
  switch (type) {
    ...
    case 'textarea':
      ...
      ename = 'oninput';
      eObj = eGroup[0]; //如果是非受控组件那么我们需要阻止用户改变数据
...
"use strict";
/* harmony export (immutable) */ __webpack_exports__["b"] = transformChildren;
/* harmony export (immutable) */ __webpack_exports__["a"] = genKey;
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0__Iteractor__ = __webpack_require__(8);
...
function transformChildren(renderedVnode, parent) {
  ...
  var result = children ? {} : null; //如有undefined null 简单数据类型合并 noCount++;
    ...
    switch (__type) {
      case 'object':
        if (c.type) {
          ...
          var iteratorFn = Object(__WEBPACK_IMPORTED_MODULE_0__Iteractor__["b" /* getIteractor */])(c);

          if (iteratorFn) {
            var ret = Object(__WEBPACK_IMPORTED_MODULE_0__Iteractor__["a" /* callIteractor */])(iteratorFn, c);
...
"use strict";
/* unused harmony export REAL_SYMBOL */
/* unused harmony export FAKE_SYMBOL */
/* harmony export (immutable) */ __webpack_exports__["b"] = getIteractor;
/* harmony export (immutable) */ __webpack_exports__["a"] = callIteractor;
...
function createElement(type, config, children) {
  ...
  var mtype = null;
  var getContext = null;
  var len = arguments.length - 2; //0代表没有ref 1代表ref为func 2为string

  var refType = 0; //既然render的时候都需要判断下type 是fun或string
  ...
  switch (_type) {
    ...
    case 'function':
      //component 或statelessComponent
      mtype = 2; //如果有contextTypes代表该组件可以取context
...
var Vnode = function (type, key, ref, props, mtype, getContext, refType) {
  ...
  this.props = props;
  this.$$typeof = 1;
  this.mtype = mtype; //之前是直接赋在vnode上 多了之后容易混 
  //单独新建个对象存放各种信息
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0__may_dom_MayDom__ = __webpack_require__(4);
...
function Component(props, context, key, ref) {
  ...
  this.key = key;
  this.ref = ref;
  this.context = context; //新建个对象存放各种信息
...
Component.prototype.setState = function (state, callback) {
  ...
  if (callback) {
    ...
    __WEBPACK_IMPORTED_MODULE_2__may_dom_scheduler__["b" /* mayQueue */].callbackQueue.push(callback);
  }
  ...
  if (__WEBPACK_IMPORTED_MODULE_2__may_dom_scheduler__["b" /* mayQueue */].isInEvent) {
    //如果在绑定事件中 触发setState合并state
    if (__WEBPACK_IMPORTED_MODULE_2__may_dom_scheduler__["b" /* mayQueue */].dirtyComponentsQueue.indexOf(this) === -1) {
      this.mayInst.dirty = true;
      __WEBPACK_IMPORTED_MODULE_2__may_dom_scheduler__["b" /* mayQueue */].dirtyComponentsQueue.push(this);
    }
  ...
  switch (lifeState) {
    ...
    case 1:
      //componentWillMount 触发setState会合并state
      return;
    ...
    case 3:
    case 2:
      //componentDidMount 触发setState会放到下一周期  2
      if (__WEBPACK_IMPORTED_MODULE_2__may_dom_scheduler__["b" /* mayQueue */].dirtyComponentsQueue.indexOf(this) === -1) {
        this.mayInst.dirty = true;
  ...
  __WEBPACK_IMPORTED_MODULE_2__may_dom_scheduler__["b" /* mayQueue */].clearQueue();
};

Component.prototype.forceUpdate = function (callback) {
  if (callback) {
    __WEBPACK_IMPORTED_MODULE_2__may_dom_scheduler__["b" /* mayQueue */].callbackQueue.push(callback.bind(this));
  }
...
"use strict";
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "a", function() { return mountStrategy; });
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0__diffProps__ = __webpack_require__(5);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1__Refs__ = __webpack_require__(2);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_2__util__ = __webpack_require__(0);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_3__scheduler__ = __webpack_require__(1);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_4__DOMNamespaces__ = __webpack_require__(6);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_5__context__ = __webpack_require__(3);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_6__transformChildren__ = __webpack_require__(7);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_7__Iteractor__ = __webpack_require__(8);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_8__instantiateComponent__ = __webpack_require__(15);
...
function mountDOM(vnode, isSVG) {
  ...
  var hostNode = !isSVG ? document.createElement(vtype) : document.createElementNS(__WEBPACK_IMPORTED_MODULE_4__DOMNamespaces__["a" /* NAMESPACE */].svg, vnode.type);

  if (__WEBPACK_IMPORTED_MODULE_1__Refs__["a" /* Refs */].isRoot) {
    __WEBPACK_IMPORTED_MODULE_1__Refs__["a" /* Refs */].currentOwner = hostNode;
    hostNode.refs = {};
    __WEBPACK_IMPORTED_MODULE_1__Refs__["a" /* Refs */].isRoot = false;
  }

  vnode.mayInfo.hostNode = hostNode;
  Object(__WEBPACK_IMPORTED_MODULE_0__diffProps__["b" /* diffProps */])(null, vnode);
  ...
  for (let i = 0; i < len; i++) {
    ...
    switch (type) {
      ...
      case 'object':
        //vnode
        if (c.type) {
          c.context = Object(__WEBPACK_IMPORTED_MODULE_5__context__["b" /* getContextByTypes */])(vnode.context, c.type.contextTypes);
          cdom = mountStrategy[c.mtype](c, isSVG);
          c.mtype === 2 && (c.mayInfo.instance.mayInst.hostNode = cdom);
          hostNode.appendChild(cdom);
        } else {
          //有可能是子数组iterator
          var iteratorFn = Object(__WEBPACK_IMPORTED_MODULE_7__Iteractor__["b" /* getIteractor */])(c);

          if (iteratorFn) {
            var ret = Object(__WEBPACK_IMPORTED_MODULE_7__Iteractor__["a" /* callIteractor */])(iteratorFn, c);
  ...
  vnode.mayInfo.vChildren = Object(__WEBPACK_IMPORTED_MODULE_6__transformChildren__["b" /* transformChildren */])(vnode, hostNode);

  if (__WEBPACK_IMPORTED_MODULE_0__diffProps__["a" /* FormElement */][vtype]) {
    //如果是受控组件input select之类需要特殊处理下
    if (vnode.props) {
      var _val = vnode.props['value'] || vnode.props['defaultValue'] || '';

      Object(__WEBPACK_IMPORTED_MODULE_0__diffProps__["c" /* getIsControlled */])(hostNode, vnode);
  ...
  if (vnode.ref) {
    __WEBPACK_IMPORTED_MODULE_1__Refs__["a" /* Refs */].attachRef(vnode, hostNode);
  }
...
function mountComposite(vnode, isSVG) {
  var hostNode = null;
  var rendered = Object(__WEBPACK_IMPORTED_MODULE_8__instantiateComponent__["a" /* buildComponentFromVnode */])(vnode);
  var inst = vnode.mayInfo.instance;

  if (!inst.mayInst.stateless && __WEBPACK_IMPORTED_MODULE_1__Refs__["a" /* Refs */].isRoot) {
    __WEBPACK_IMPORTED_MODULE_1__Refs__["a" /* Refs */].currentOwner = inst;
    inst.refs = {};
    __WEBPACK_IMPORTED_MODULE_1__Refs__["a" /* Refs */].isRoot = false;
  }
  ...
  if (rendered) {
    ...
    hostNode = mountStrategy[rendered.mtype](rendered, isSVG); //dom diff需要分类一下children以方便diff

    rendered.mayInfo.vChildren = Object(__WEBPACK_IMPORTED_MODULE_6__transformChildren__["b" /* transformChildren */])(rendered, hostNode); // rendered.mayInfo.hostNode = hostNode;
  } else {
    //render 返回null
    hostNode = document.createComment('empty');
    vnode.mayInfo.hostNode = hostNode; //用于isMounted 判断 即使是null

    inst.mayInst.isEmpty = true;
  }

  if (inst.componentDidMount) {
    __WEBPACK_IMPORTED_MODULE_3__scheduler__["a" /* lifeCycleQueue */].push(inst.componentDidMount.bind(inst));
  } else {
  ...
  if (vnode.ref) {
    __WEBPACK_IMPORTED_MODULE_1__Refs__["a" /* Refs */].attachRef(vnode, inst);
  }
...
function mountText(vnode) {
  if (vnode) {
    var node = __WEBPACK_IMPORTED_MODULE_2__util__["c" /* recyclables */]['#text'].pop();
...
"use strict";
/* harmony export (immutable) */ __webpack_exports__["b"] = disposeVnode;
/* unused harmony export disposeComponent */
/* harmony export (immutable) */ __webpack_exports__["a"] = disposeDom;
/* harmony export (immutable) */ __webpack_exports__["c"] = emptyElement;
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0__util__ = __webpack_require__(0);
...
function disposeDom(dom) {
  ...
  if (dom.nodeType === 1) {
    ...
    if (__WEBPACK_IMPORTED_MODULE_0__util__["c" /* recyclables */]['#text'].length < 100) {
      __WEBPACK_IMPORTED_MODULE_0__util__["c" /* recyclables */]['#text'].push(dom);
    }
  }
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0__MayElement__ = __webpack_require__(9);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1__Component__ = __webpack_require__(10);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_2__PureComponent__ = __webpack_require__(17);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_3__cloneElement__ = __webpack_require__(18);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_4__Children__ = __webpack_require__(19);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_5__PropTypes__ = __webpack_require__(20);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_6__may_dom_MayDom__ = __webpack_require__(4);
...
var May = {
  createElement: __WEBPACK_IMPORTED_MODULE_0__MayElement__["a" /* createElement */],
  Component: __WEBPACK_IMPORTED_MODULE_1__Component__["a" /* Component */],
  PureComponent: __WEBPACK_IMPORTED_MODULE_2__PureComponent__["a" /* PureComponent */],
  cloneElement: __WEBPACK_IMPORTED_MODULE_3__cloneElement__["a" /* cloneElement */],
  Children: __WEBPACK_IMPORTED_MODULE_4__Children__["a" /* Children */],
  render: __WEBPACK_IMPORTED_MODULE_6__may_dom_MayDom__["c" /* render */],
  PropTypes: __WEBPACK_IMPORTED_MODULE_5__PropTypes__["a" /* PropTypes */],
  findDOMNode: __WEBPACK_IMPORTED_MODULE_6__may_dom_MayDom__["a" /* findDOMNode */],
  unmountComponentAtNode: __WEBPACK_IMPORTED_MODULE_6__may_dom_MayDom__["d" /* unmountComponentAtNode */],
  isValidElement: function (vnode) {
    return vnode && vnode.mtype;
  },
  createFactory: function createFactory(type) {
    console.error("createFactory is deprecated");
    var factory = __WEBPACK_IMPORTED_MODULE_0__MayElement__["a" /* createElement */].bind(null, type);
    factory.type = type;
    return factory;
  }
};
window.React = window.ReactDOM = May;
/* harmony default export */ __webpack_exports__["default"] = (May);
...
"use strict";
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "b", function() { return eventHooks; });
/* unused harmony export isTouch */
/* unused harmony export dispatchEvent */
/* unused harmony export eventLowerCache */
/* harmony export (immutable) */ __webpack_exports__["c"] = getBrowserName;
/* harmony export (immutable) */ __webpack_exports__["a"] = addEvent;
/* unused harmony export SyntheticEvent */
...
function dispatchEvent(e, type, end) {
  ...
  var _type = e.type; //全局的一个标识  在事件中setState应当合并

  __WEBPACK_IMPORTED_MODULE_0__may_dom_scheduler__["b" /* mayQueue */].isInEvent = true; //onClickCapture 在捕获阶段触发

  var captured = _type + 'capture';
  var eventCollect = bubbleEvent(e.target, end || document); //先触发捕获
  ...
  __WEBPACK_IMPORTED_MODULE_0__may_dom_scheduler__["b" /* mayQueue */].isInEvent = false; //在事件中合并state之后 触发reRender

  __WEBPACK_IMPORTED_MODULE_0__may_dom_scheduler__["b" /* mayQueue */].clearQueue();
}
/**
 * 自己冒泡,收集冒泡过程中的所有事件
...
function bubbleEvent(from, end) {
  ...
  } while ((from = from.parentNode) && from.nodeType === 1); // target --> parentNode --> body --> html
    ...
    if (!dom[mark]) {
      ...
      addDocumentEvent(dom, mask, function (e) {
        ...
        if (!t || t !== dom && !contains(dom, t)) {
          var common = getLowestCommonAncestor(dom, t); //由于不冒泡，因此paths长度为1
...
"use strict";
/* harmony export (immutable) */ __webpack_exports__["a"] = buildComponentFromVnode;
...
function buildComponentFromVnode(vnode) {
  ...
  var inst, rendered;
  var Ctor = vnode.type; //Component  PureComponent

  if (Ctor.prototype && Ctor.prototype.render) {
    //props, context需要放在前俩
    inst = new Ctor(props, context, key, ref); //constructor里面props不可变

    inst.props = props;
    inst.refType = vnode.refType;
    inst.mayInst.mountOrder = mountOrder;
    mountOrder++; //_lifeState来控制生命周期中调用setState的作用
    ...
    if (inst.mayInst.mergeStateQueue) {
      inst.state = Object(__WEBPACK_IMPORTED_MODULE_1__util__["b" /* mergeState */])(inst);
    } //为2代表开始render
    ...
    if (inst.getChildContext) {
      context = Object(__WEBPACK_IMPORTED_MODULE_0__context__["a" /* getChildContext */])(inst, context);
    }

    if (vnode.getContext) {
      inst.context = Object(__WEBPACK_IMPORTED_MODULE_0__context__["b" /* getContextByTypes */])(context, Ctor.contextTypes);
    }
    ...
    rendered = inst.render.call(vnode, Ctor); //should support module pattern components
...
"use strict";
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "b", function() { return updateStrategy; });
/* unused harmony export diffChildren */
/* harmony export (immutable) */ __webpack_exports__["a"] = isSameType;
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0__dispose__ = __webpack_require__(12);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1__diffProps__ = __webpack_require__(5);
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_4__scheduler__ = __webpack_require__(1);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_5__transformChildren__ = __webpack_require__(7);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_6__Refs__ = __webpack_require__(2);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_7__DOMNamespaces__ = __webpack_require__(6);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_8__mountStrategy__ = __webpack_require__(11);
...
function updateDOM(prevVnode, newVnode) {
  ...
  if (__WEBPACK_IMPORTED_MODULE_6__Refs__["a" /* Refs */].isRoot) {
    __WEBPACK_IMPORTED_MODULE_6__Refs__["a" /* Refs */].currentOwner = hostNode;
    hostNode.refs = {};
    __WEBPACK_IMPORTED_MODULE_6__Refs__["a" /* Refs */].isRoot = false;
  }

  var isSVG = hostNode && hostNode.namespaceURI === __WEBPACK_IMPORTED_MODULE_7__DOMNamespaces__["a" /* NAMESPACE */].svg;

  if (isSVG) {
    newVnode.mayInfo.isSVG = true;
  }

  Object(__WEBPACK_IMPORTED_MODULE_1__diffProps__["b" /* diffProps */])(prevVnode, newVnode);
  diffChildren(prevVnode, newVnode, hostNode);
  newVnode.mayInfo.vChildren = Object(__WEBPACK_IMPORTED_MODULE_5__transformChildren__["b" /* transformChildren */])(newVnode, hostNode);

  if (__WEBPACK_IMPORTED_MODULE_1__diffProps__["a" /* FormElement */][vtype]) {
    //如果是受控组件input select之类需要特殊处理下
    if (newVnode.props) {
      var isControlled = Object(__WEBPACK_IMPORTED_MODULE_1__diffProps__["c" /* getIsControlled */])(hostNode, newVnode);
  ...
  if (newVnode.ref) {
    __WEBPACK_IMPORTED_MODULE_6__Refs__["a" /* Refs */].attachRef(newVnode, hostNode);
  }
...
function updateComposite(prevVnode, newVnode) {
  ...
  if (newVnode.getContext) {
    context = Object(__WEBPACK_IMPORTED_MODULE_2__context__["b" /* getContextByTypes */])(context, newVnode.type.contextTypes);
    temporaryContext = context;
  }

  var instance = prevVnode.mayInfo.instance;
  var prevRendered = instance.mayInst.rendered;
  var hostNode = prevRendered && prevRendered.mtype === 1 ? prevRendered.mayInfo.hostNode : instance.mayInst.hostNode; //empty代表prevVnode为null
  ...
  if (!instance.mayInst.stateless) {
    ...
    prevProps = prevVnode.props; //lifeState为3组件开始diff
    //WillReceive WillUpdate render DidUpdate等周期setState放到下一周期;

    instance.mayInst.lifeState = 3; //用于mergeState如果setState传入一个function s.call(instance, newState, instance.nextProps || instance.props);
    ...
    if (instance.getChildContext) {
      ...
      context = Object(__WEBPACK_IMPORTED_MODULE_2__context__["a" /* getChildContext */])(instance, context);
    }

    var newState = Object(__WEBPACK_IMPORTED_MODULE_3__util__["b" /* mergeState */])(instance); //如果context与props都没有改变，那么就不会触发组件的receive，render，update等一系列钩子
    //但还会继续向下比较
    ...
    if (needReceive) {
      if (instance.componentWillReceiveProps) {
        ...
        if (instance.mayInst.mergeStateQueue && instance.mayInst.mergeStateQueue.length > 0) {
          newState = Object(__WEBPACK_IMPORTED_MODULE_3__util__["b" /* mergeState */])(instance);
        } else {
          //this.state={///}的情况
      ...
      var rendered = instance.mayInst.rendered; //context穿透更新问题
    ...
    if (__WEBPACK_IMPORTED_MODULE_6__Refs__["a" /* Refs */].isRoot) {
      __WEBPACK_IMPORTED_MODULE_6__Refs__["a" /* Refs */].currentOwner = instance;
      __WEBPACK_IMPORTED_MODULE_6__Refs__["a" /* Refs */].currentOwner.refs = {};
      __WEBPACK_IMPORTED_MODULE_6__Refs__["a" /* Refs */].isRoot = false;
    }
    ...
    if (!isEmpty && newRendered) {
      if (isSameType(prevRendered, newRendered)) {
        ...
        Object(__WEBPACK_IMPORTED_MODULE_0__dispose__["b" /* disposeVnode */])(prevRendered);
        var isSVG = newRendered.mtype === 3;
        newDom = __WEBPACK_IMPORTED_MODULE_8__mountStrategy__["a" /* mountStrategy */][newRendered.mtype](newRendered, isSVG);
    ...
    if (instance.componentDidUpdate) {
      __WEBPACK_IMPORTED_MODULE_4__scheduler__["a" /* lifeCycleQueue */].push(instance.componentDidUpdate.bind(instance, prevProps, prevState, instance.context));
    } else {
      //如果没有回调则其render生命周期结束lifeState为0
      instance.mayInst.lifeState = 0;
    }
    ...
    if (newVnode.refType === 1) {
      __WEBPACK_IMPORTED_MODULE_6__Refs__["a" /* Refs */].attachRef(newVnode, instance);
    }
  } else {
    //stateless component
    ...
    if (prevRendered && isSameType(prevRendered, newRendered)) {
      ...
      Object(__WEBPACK_IMPORTED_MODULE_0__dispose__["b" /* disposeVnode */])(prevRendered);
      var isSVG = newVnode.mtype === 3;
      newDom = __WEBPACK_IMPORTED_MODULE_8__mountStrategy__["a" /* mountStrategy */][newVnode.mtype](newVnode, isSVG);
...
function diffChildren(prevVnode, updatedVnode, parent) {
  ...
  if (newRenderedChild) {
    ...
    for (var i = 0; i < len; i++) {
      ...
      switch (t) {
        case 'object':
          k = Object(__WEBPACK_IMPORTED_MODULE_5__transformChildren__["a" /* genKey */])(c); //

          if (c.type && c.type.contextTypes) {
            c.context = Object(__WEBPACK_IMPORTED_MODULE_2__context__["b" /* getContextByTypes */])(updatedVnode.context, c.type.contextTypes);
          } else {
            c.context = {}; //该组件没有定义contextTypes 无法使用context
            //我们还是需要保存一份context
...
function flushMounts(newChildren, parent) {
  for (var _i = 0; _i < newChildren.length; _i++) {
    ...
    if (child.mayInfo.prevVnode) {
      ...
      //新增节点
      var isSVG = child.mtype === 3; // var isSVG = vnode.namespaceURI === "http://www.w3.org/2000/svg";

      newDom = __WEBPACK_IMPORTED_MODULE_8__mountStrategy__["a" /* mountStrategy */][child.mtype](child, isSVG);
      child.mtype === 2 && (child.mayInfo.instance.mayInst.hostNode = newDom); // child.mayInfo.hostNode = newDom;
...
function flushUnMounts(oldChildren) {
  ...
  while (c = oldChildren.shift()) {
    ...
    Object(__WEBPACK_IMPORTED_MODULE_0__dispose__["a" /* disposeDom */])(dom);
    Object(__WEBPACK_IMPORTED_MODULE_0__dispose__["b" /* disposeVnode */])(c);
    c = null;
  }
}
...
"use strict";
/* harmony export (immutable) */ __webpack_exports__["a"] = PureComponent;
...
function PureComponent(props, key, ref, context) {
  return __WEBPACK_IMPORTED_MODULE_0__Component__["a" /* Component */].apply(this, arguments);
}
var fn = Object(__WEBPACK_IMPORTED_MODULE_1__util__["a" /* inherits */])(PureComponent, __WEBPACK_IMPORTED_MODULE_0__Component__["a" /* Component */]); //返回false 则不进行之后的渲染
...
function cloneElement(element, additionalProps) {
  ...
  var ret = Object(__WEBPACK_IMPORTED_MODULE_0__MayElement__["a" /* createElement */])(type, config, children);
  return ret;
}
...
"use strict";
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0__may_dom_Iteractor__ = __webpack_require__(8);
  ...
  if (Array.isArray(children)) {
    for (var i = 0; i < children.length; i++) {
      ...
      if (c.type) {
        ...
        //有可能是子数组iterator
        var iteratorFn = Object(__WEBPACK_IMPORTED_MODULE_0__may_dom_Iteractor__["b" /* getIteractor */])(c);
