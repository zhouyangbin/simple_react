...
exports.recyclables = void 0;
//文本节点重复利用
var recyclables = {
  '#text': []
};
exports.recyclables = recyclables;
...
exports.lifeCycleQueue = exports.mayQueue = void 0;

var _MayDom = __webpack_require__(4);

var _Refs = __webpack_require__(2);
...
var Refs = {
  ...
  isRoot: false,
  attachRef: function attachRef(vnode, hostNode) {
    if (vnode.refType === 1) {
      //func
      hostNode.mayInst && hostNode.mayInst.stateless && (hostNode = null);

      _scheduler.lifeCycleQueue.push(vnode.ref.bind(vnode, hostNode));
...
exports.getContextByTypes = getContextByTypes;
...
function getChildContext(instance, context) {
  ...
  if (getContext && _typeof(getContext) === 'object') {
    if (!context) {
      context = {};
    }
...
var _mountStrategy = __webpack_require__(11);

var _diffStrategy = __webpack_require__(16);

var _diffProps = __webpack_require__(5);

var _Refs = __webpack_require__(2);

var _DOMNamespaces = __webpack_require__(6);

var _context = __webpack_require__(3);

var _transformChildren = __webpack_require__(7);

var _dispose = __webpack_require__(12);
...
var renderByMay = function renderByMay(vnode, container, callback) {
  ...
  if (lastVnode) {
    ...
    if (vnode && vnode.type) {
      ...
      _Refs.Refs.isRoot = true;
      rootDom = _mountStrategy.mountStrategy[vnode.mtype](vnode, isSVG);
  ...
  _scheduler.mayQueue.clearQueue();
...
function reRender(instance) {
  ...
  var newState = (0, _util.mergeState)(instance); //forceUpdate时 忽略shouldComponentUpdate
  ...
  if (instance.getChildContext) {
    context = (0, _context.getChildContext)(instance, context);
  }
  ...
  if (_Refs.Refs.isRoot) {
    _Refs.Refs.currentOwner = instance;
    _Refs.Refs.isRoot = false;
  }
  ...
  if (!updated) {
    (0, _dispose.disposeVnode)(prevRendered);
    return;
  }

  if (!isEmpty && (0, _diffStrategy.isSameType)(prevRendered, updated)) {
    // updated.mayInfo.instance = instance;
    // updated.mayInfo.hostNode = hostNode;
    hostNode = _diffStrategy.updateStrategy[updated.mtype](prevRendered, updated); //mtype === 1在这在设置instance会循环引用
    // updated.mtype === 2 && ();

    updated.mayInfo.vChildren = (0, _transformChildren.transformChildren)(updated, hostNode);
    ...
    var dom = _mountStrategy.mountStrategy[updated.mtype](updated, isSVG); //component上的hostNode保持最新
...
function mayUpdate(prevVnode, newVnode, parent) {
  ...
  if ((0, _diffStrategy.isSameType)(prevVnode, newVnode)) {
    dom = _diffStrategy.updateStrategy[newVnode.mtype](prevVnode, newVnode);
  } else {
    var isSVG = newVnode.mtype === 3;
    dom = _mountStrategy.mountStrategy[newVnode.mtype](newVnode, isSVG);
    ...
    (0, _dispose.disposeVnode)(prevVnode);
    hostNode = null;
...
function unmountComponentAtNode(dom) {
  ...
  if (lastVnode) {
    (0, _dispose.disposeVnode)(lastVnode);
    (0, _dispose.emptyElement)(dom); //unmount之后又render
...
exports.removeDomAttr = removeDomAttr;
exports.patchStyle = patchStyle;
exports.getIsControlled = getIsControlled;
...
function setDomAttr(dom, key, val) {
  ...
  if (!isEvent(key)) {
    ...
    var eventName = (0, _event.getBrowserName)(key);
    var listener = dom._listener || (dom._listener = {});

    if (!listener[e]) {
      //添加过一次之后不必再添加;
      (0, _event.addEvent)(eventName);
      var hook = _event.eventHooks[eventName];
...
function patchStyle(dom, prevStyle, newStyle) {
  ...
  for (var name in newStyle) {
    var _type = _typeof(newStyle[name]); //backgroundColor 替换为 background-color Webkit替换为-webkit-   ms单独替换一次
...
var _Iteractor = __webpack_require__(8);
    ...
    switch (__type) {
      case 'object':
        if (c.type) {
          ...
          var iteratorFn = (0, _Iteractor.getIteractor)(c);

          if (iteratorFn) {
            var ret = (0, _Iteractor.callIteractor)(iteratorFn, c);
      ...
      case 'string':
        ...
        if (i + 1 < len) {
          var _ntype = _typeof(children[i + 1 - noCount]);
...
exports.getIteractor = getIteractor;
exports.callIteractor = callIteractor;
exports.FAKE_SYMBOL = exports.REAL_SYMBOL = void 0;
var REAL_SYMBOL = typeof Symbol === "function" && Symbol.iterator;
exports.REAL_SYMBOL = REAL_SYMBOL;
var FAKE_SYMBOL = "@@iterator";
exports.FAKE_SYMBOL = FAKE_SYMBOL;
...
var Vnode = function Vnode(type, key, ref, props, mtype, getContext, refType) {
  this.type = type;
  this.key = key;
  this.ref = ref;
  this.props = props;
...
Component.prototype.setState = function (state, callback) {
  ...
  if (callback) {
    ...
    _scheduler.mayQueue.callbackQueue.push(callback);
  }
  ...
  if (_scheduler.mayQueue.isInEvent) {
    //如果在绑定事件中 触发setState合并state
    if (_scheduler.mayQueue.dirtyComponentsQueue.indexOf(this) === -1) {
      this.mayInst.dirty = true;

      _scheduler.mayQueue.dirtyComponentsQueue.push(this);
    }
...
Component.prototype.forceUpdate = function (callback) {
  if (callback) {
    _scheduler.mayQueue.callbackQueue.push(callback.bind(this));
  }
...
exports.mountStrategy = mountStrategy;

function mountDOM(vnode, isSVG) {
  var vtype = vnode.type;
  vnode.mayInfo.isSVG = isSVG;
  var hostNode = !isSVG ? document.createElement(vtype) : document.createElementNS(_DOMNamespaces.NAMESPACE.svg, vnode.type);

  if (_Refs.Refs.isRoot) {
    _Refs.Refs.currentOwner = hostNode;
    hostNode.refs = {};
    _Refs.Refs.isRoot = false;
  }

  vnode.mayInfo.hostNode = hostNode;
  (0, _diffProps.diffProps)(null, vnode);
  ...
  for (var i = 0; i < len; i++) {
    ...
    switch (type) {
      ...
      case 'object':
        //vnode
        if (c.type) {
          c.context = (0, _context.getContextByTypes)(vnode.context, c.type.contextTypes);
  ...
  vnode.mayInfo.vChildren = (0, _transformChildren.transformChildren)(vnode, hostNode);

  if (_diffProps.FormElement[vtype]) {
    //如果是受控组件input select之类需要特殊处理下
    if (vnode.props) {
      var _val = vnode.props['value'] || vnode.props['defaultValue'] || '';

      (0, _diffProps.getIsControlled)(hostNode, vnode);
  ...
  if (vnode.ref) {
    _Refs.Refs.attachRef(vnode, hostNode);
  }
...
function mountComposite(vnode, isSVG) {
  var hostNode = null;
  var rendered = (0, _instantiateComponent.buildComponentFromVnode)(vnode);
  var inst = vnode.mayInfo.instance;

  if (!inst.mayInst.stateless && _Refs.Refs.isRoot) {
    _Refs.Refs.currentOwner = inst;
    inst.refs = {};
    _Refs.Refs.isRoot = false;
  }
  ...
  if (rendered) {
    ...
    rendered.mayInfo.vChildren = (0, _transformChildren.transformChildren)(rendered, hostNode); // rendered.mayInfo.hostNode = hostNode;
  } else {
    //render 返回null
  ...
  if (inst.componentDidMount) {
    _scheduler.lifeCycleQueue.push(inst.componentDidMount.bind(inst));
  } else {
  ...
  if (vnode.ref) {
    _Refs.Refs.attachRef(vnode, inst);
  }
...
function mountText(vnode) {
  if (vnode) {
    var node = _util.recyclables['#text'].pop();
...
function disposeDom(dom) {
  ...
  if (dom.nodeType === 1) {
    ...
    if (_util.recyclables['#text'].length < 100) {
      _util.recyclables['#text'].push(dom);
    }
  }
...
var _MayElement = __webpack_require__(9);

var _Component = __webpack_require__(10);

var _PureComponent = __webpack_require__(17);
...
var May = {
  createElement: _MayElement.createElement,
  Component: _Component.Component,
  PureComponent: _PureComponent.PureComponent,
  cloneElement: _cloneElement.cloneElement,
  Children: _Children.Children,
  render: _MayDom.render,
  PropTypes: _PropTypes.PropTypes,
  findDOMNode: _MayDom.findDOMNode,
  unmountComponentAtNode: _MayDom.unmountComponentAtNode,
  ...
  createFactory: function createFactory(type) {
    ...
    var factory = _MayElement.createElement.bind(null, type);
...
exports.eventLowerCache = exports.isTouch = exports.eventHooks = void 0;
...
function dispatchEvent(e, type, end) {
  ...
  _scheduler.mayQueue.isInEvent = true; //onClickCapture 在捕获阶段触发
  ...
  _scheduler.mayQueue.isInEvent = false; //在事件中合并state之后 触发reRender
...
exports.eventLowerCache = eventLowerCache;
...
function buildComponentFromVnode(vnode) {
  ...
  if (Ctor.prototype && Ctor.prototype.render) {
    ...
    if (inst.mayInst.mergeStateQueue) {
      inst.state = (0, _util.mergeState)(inst);
    } //为2代表开始render
    ...
    if (inst.getChildContext) {
      context = (0, _context.getChildContext)(inst, context);
    }

    if (vnode.getContext) {
      inst.context = (0, _context.getContextByTypes)(context, Ctor.contextTypes);
    }
...
function updateDOM(prevVnode, newVnode) {
  ...
  var isSVG = hostNode && hostNode.namespaceURI === _DOMNamespaces.NAMESPACE.svg;

  if (isSVG) {
    newVnode.mayInfo.isSVG = true;
  }

  (0, _diffProps.diffProps)(prevVnode, newVnode);
  diffChildren(prevVnode, newVnode, hostNode);
  newVnode.mayInfo.vChildren = (0, _transformChildren.transformChildren)(newVnode, hostNode);

  if (_diffProps.FormElement[vtype]) {
    //如果是受控组件input select之类需要特殊处理下
    if (newVnode.props) {
      var isControlled = (0, _diffProps.getIsControlled)(hostNode, newVnode);
  ...
  if (newVnode.ref) {
    _Refs.Refs.attachRef(newVnode, hostNode);
  }
...
function updateComposite(prevVnode, newVnode) {
  ...
  if (newVnode.getContext) {
    context = (0, _context.getContextByTypes)(context, newVnode.type.contextTypes);
  ...
  if (!instance.mayInst.stateless) {
    ...
    var newState = (0, _util.mergeState)(instance); //如果context与props都没有改变，那么就不会触发组件的receive，render，update等一系列钩子
    //但还会继续向下比较
    ...
    if (needReceive) {
      if (instance.componentWillReceiveProps) {
        ...
        if (instance.mayInst.mergeStateQueue && instance.mayInst.mergeStateQueue.length > 0) {
          newState = (0, _util.mergeState)(instance);
        } else {
          //this.state={///}的情况
    ...
    if (!isEmpty && newRendered) {
      if (isSameType(prevRendered, newRendered)) {
        ...
        newDom = _mountStrategy.mountStrategy[newRendered.mtype](newRendered, isSVG);
    ...
    if (instance.componentDidUpdate) {
      _scheduler.lifeCycleQueue.push(instance.componentDidUpdate.bind(instance, prevProps, prevState, instance.context));
    } else {
      //如果没有回调则其render生命周期结束lifeState为0
      instance.mayInst.lifeState = 0;
    }
    ...
    if (newVnode.refType === 1) {
      _Refs.Refs.attachRef(newVnode, instance);
    }
  } else {
    //stateless component
    ...
    if (prevRendered && isSameType(prevRendered, newRendered)) {
      ...
      (0, _dispose.disposeVnode)(prevRendered);
      var isSVG = newVnode.mtype === 3;
      newDom = _mountStrategy.mountStrategy[newVnode.mtype](newVnode, isSVG);
...
function diffChildren(prevVnode, updatedVnode, parent) {
  ...
  if (newRenderedChild) {
    ...
    for (var i = 0; i < len; i++) {
      ...
      switch (t) {
        case 'object':
          k = (0, _transformChildren.genKey)(c); //

          if (c.type && c.type.contextTypes) {
            c.context = (0, _context.getContextByTypes)(updatedVnode.context, c.type.contextTypes);
          } else {
        ...
        case 'string':
          ...
          if (i + 1 < newRenderedChild.length) {
            var _ntype = _typeof(newRenderedChild[i + 1]);
...
function flushMounts(newChildren, parent) {
  for (var _i = 0; _i < newChildren.length; _i++) {
    ...
    if (child.mayInfo.prevVnode) {
      ...
      newDom = _mountStrategy.mountStrategy[child.mtype](child, isSVG);
...
function flushUnMounts(oldChildren) {
  ...
  while (c = oldChildren.shift()) {
    ...
    (0, _dispose.disposeDom)(dom);
    (0, _dispose.disposeVnode)(c);
    c = null;
  }
}
...
function PureComponent(props, key, ref, context) {
  return _Component.Component.apply(this, arguments);
}

var fn = (0, _util.inherits)(PureComponent, _Component.Component); //返回false 则不进行之后的渲染
...
function shallowEqual(now, next) {
  ...
  if (now && _typeof(now) !== 'object' || next && _typeof(next) !== 'object') {
    return false;
  }
...
function cloneElement(element, additionalProps) {
  ...
  var ret = (0, _MayElement.createElement)(type, config, children);
  return ret;
}
...
var Children = {
  ...
  map: function map(children, callback, context) {
    ...
    _toArray(children).forEach(function (item, index) {
  ...
  toArray: function toArray(children) {
    ...
    return _toArray(children);
  }
};
exports.Children = Children;

function _toArray(children) {
  var ret = [];
